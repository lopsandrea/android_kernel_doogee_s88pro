...
BYTE IMX230XINFENGDA_DCC_data[96] = { 0 };
...
#endif
static bool _read_imx230_xinfengda_eeprom(kal_uint16 addr, BYTE *data, int size)
{
	int i = 0;
	int offset = addr;
	int ret;
...
void read_imx230_xinfengda_SPC(BYTE *data)
{
	...
	if (!get_done_spc || last_size_spc != size) {
		if (!_read_imx230_xinfengda_eeprom(addr, data, size)) {
			get_done_spc = 0;
			last_size_spc = 0;
		}
	}
...
void read_imx230_xinfengda_DCC(kal_uint16 addr, BYTE *data, kal_uint32 size)
{
	/* int i; */
	addr = DCC_START_ADDR;
	size = 96;
	...
	if (!get_done_dcc || last_size_dcc != size) {
		if (!_read_imx230_xinfengda_eeprom(addr, IMX230XINFENGDA_DCC_data, size)) {
			get_done_dcc = 0;
			last_size_dcc = 0;
		}
	}

	memcpy(data, IMX230XINFENGDA_DCC_data, size);
	/* return true; */
}
