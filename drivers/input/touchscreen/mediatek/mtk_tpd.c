...
#include <linux/fb.h>
/*
 * for the proc_create/single_open/seq_printf of the four Wingtech
 * functions added further down -- OUTSIDE the conditional #ifs this
 * file uses later on for <linux/input/mt.h>
 */
...
/*
 * This section was reconstructed from the factory kernel disassembly (0xffffff8008a5151c, 204 bytes).
 *
 * The working notes -- the disassembly citations, the measurements against
 * the factory binary and the reasoning behind each choice -- are in
 * docs/bringup/verbali-driver/drivers_input_touchscreen_mediatek_mtk_tpd.md
 * in the oracolo repository. They are kept in Italian, as the project's
 * internal record.
 */

/*
 * This section was reconstructed from the factory kernel disassembly (0xffffff8008a5a190, 43 bytes).
 *
 * The working notes -- the disassembly citations, the measurements against
 * the factory binary and the reasoning behind each choice -- are in
 * docs/bringup/verbali-driver/drivers_input_touchscreen_mediatek_mtk_tpd.md
 ...
 */
/*
 * This section was reconstructed from the factory kernel disassembly (0xffffff800a0fc0b8).
 *
 * The working notes -- the disassembly citations, the measurements against
 * the factory binary and the reasoning behind each choice -- are in
 * docs/bringup/verbali-driver/drivers_input_touchscreen_mediatek_mtk_tpd.md
...
/*
 * "92401c33 and"@0xffffff8008a51530 masks the second argument to EIGHT bits
 * before using it: it is a `u8`, not an `int`. "941019ef bl"@0xffffff8008a51544
 * goes to <__memcpy> with x0 = the array, x1 = the argument, x2 = the length; and
 * "38336a9f strb"@0xffffff8008a5154c writes the zero at the tail of `[x20,x19]`,
 * that is `wtk_tp_info[len]`. "2a1f03e0 mov"@0xffffff8008a51550 returns 0.
 */
...
/*
 * "97e11a92 bl"@0xffffff8008a515d8 goes to <seq_printf> with
 * x1 = "%s\n"@0xffffff8009226be1 and x2 = the array
 * ("9102e442 add"@0xffffff8008a515d4 = page + 0xb9, the same address
 * `fix_tp_proc_info` writes). "2a1f03e0 mov"@0xffffff8008a515dc returns 0.
 */
...
/*
 * "aa0103e0 mov"@0xffffff8008a515a4 passes the SECOND argument as the first to
 * <single_open> ("97e11c04 bl"@0xffffff8008a515b4): the function's first
 * argument is unused, as it is for a `file_operations` `.open`.
 * "91170021 add"@0xffffff8008a515ac sets x1 = 0xffffff8008a515c0, that is
 * `wtk_tp_info_show`, and "aa1f03e2 mov"@0xffffff8008a515b0 sets x2 = NULL.
 */
...
/*
 * This section was reconstructed from the factory kernel disassembly (0xffffff8008f7e390).
 *
 * The working notes -- the disassembly citations, the measurements against
 * the factory binary and the reasoning behind each choice -- are in
 * docs/bringup/verbali-driver/drivers_input_touchscreen_mediatek_mtk_tpd.md
 ...
 * sets NULL as the parent, and "910e5c00 add"@0xffffff8008a5156c sets
 * "wtk_tpInfo"@0xffffff800923d397. The return test is "b5000080 cbnz"
 * @0xffffff8008a51580, which SKIPS the `printk` when the pointer is non-null:
 * the message "create /proc/wtk_tpInfo fail\n"@0xffffff800923d3a2 is
 * the error branch. "2a1f03e0 mov"@0xffffff8008a51590 returns 0 in both
 * cases.
 */
...
static int __init tpd_device_init(void)
{
	...
	/*
	 * A RETRACTION. The note above said wtk_creat_proc_tp_info
	 * is called by nobody, because there is no `bl` towards
	 * 0xffffff8008a5155c. The `bl` is absent because it is INLINED here: the
	 * factory has "97bdc883 bl"@0xffffff8009383248 (proc_create) with the
	 * same string "wtk_tpInfo"@0xffffff800923d397 and the same permissions
	 * ("52802481 mov"@0xffffff8009383240, 0x124 = 0444), followed by
	 * "97b6c09f bl"@0xffffff8009383258 on the error message. Looking for a
	 * `bl` and not finding one does not prove a function is uncalled:
	 * it only proves it is not called OUT OF LINE.
	 */
